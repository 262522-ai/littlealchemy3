...
if "used_recipes" not in st.session_state:
    st.session_state.used_recipes = set()
...
with st.sidebar:
    ...
    if cheat_code == "admin123":
        if st.button("🌟 모든 원소 해금", use_container_width=True):
            ...
            st.session_state.used_recipes = set()  # 치트 시에도 초기화
            st.success("모든 원소가 해금되었습니다!")
            st.rerun()

# 5. JS → Python 동기화 (조합법 중복 방지 포함)
pending_element = st_javascript("sessionStorage.getItem('pending_new_element') || ''")
pending_key = st_javascript("sessionStorage.getItem('pending_recipe_key') || ''")

if pending_element and pending_element.strip():
    if pending_element not in st.session_state.elements:
        st.session_state.elements.append(pending_element)
        if pending_key:
            st.session_state.used_recipes.add(pending_key)
        st.toast(f"🎉 새로운 물질 발견: {pending_element}!", icon="✨")
    
    # 정리
    st_javascript("sessionStorage.removeItem('pending_new_element')")
    st_javascript("sessionStorage.removeItem('pending_recipe_key')")

# 6. 프론트엔드 (조합법 중복 방지 버전)
elements_json = json.dumps(st.session_state.elements)
recipes_json = json.dumps(RECIPES)
used_recipes_json = json.dumps(list(st.session_state.used_recipes))
...
<div style="display: flex; flex-direction: column; gap: 12px; font-family: system-ui, sans-serif;">
    <div style="display: flex; gap: 12px; align-items: center;">
        ...
        <div style="color: #868e96; font-size: 13px;">💡 한번 성공한 조합은 다시 합쳐지지 않습니다</div>
    </div>
...
<script>
    ...
    const usedRecipes = {used_recipes_json};
    ...
    function tryCombine(objA, objB) {{
        ...
        // Python에 전달
        sessionStorage.setItem('pending_recipe_key', key);
